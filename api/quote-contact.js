...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { fullName, email, phone, eventDate } = body;

    // Validate required fields
    if (!fullName || !email || !phone || !eventDate) {
      return res.status(400).json({ error: 'Full name, email, phone, and event date are required' });
    }
      ...
      <body>
        <div class="email-container">
          <div class="email-header">
            <h2>New Quote Request - Contact Info</h2>
          </div>
          <div class="email-content">
            <div class="note-box">
              <p style="margin: 0; color: #666666; font-size: 0.875rem;">
                <strong>Note:</strong> This contact information was captured from Step 1 of the quote tool. The customer may still be completing their quote.
              </p>
            </div>
          ...
          <div class="email-footer">
            <p>This email was sent from the GTA Photo Booths quote tool.</p>
    ...
    const data = await resend.emails.send({
      ...
      replyTo: email,
      subject: `New Quote Request - ${fullName} (Event: ${formattedDate})`,
      html: emailContent,
    });

    return res.status(200).json({ success: true, message: 'Contact info sent successfully', data });
  } catch (error) {
